...
def problem_one(request):
  ...
  honor_students = Student.objects.filter(gpa__gte = 3 ).order_by('-gpa')
...
def problem_two(request):
  ...
  legacy_instructors = Instructor.objects.filter(hire_date__year=2005).order_by('hire_date')

  for instructor in legacy_instructors:
...
def problem_three(request):
  ...
  find_instructor = Instructor.objects.filter(pk=2)
  
  for instructor in find_instructor:
    print(f"Instructor Name: {instructor.first_name} {instructor.last_name}")

  
  find_course = Course.objects.filter(instructor_id=2)
  print("Courses:")

  for course in find_course:
    print(f"  - {course.name}")
...
def problem_five(request):
  ...
  create_new_student = Student.objects.create(first_name="Joshua", last_name="Bischoff", gpa="3.8")
  
  
  print(f"ID: {create_new_student.id}\nFull Name: {create_new_student.first_name} {create_new_student.last_name}\nGPA: {create_new_student.gpa}")
...
def problem_six(request):
    ...
    update_student = Student.objects.filter(pk=17).update(gpa=3.9)
    get_student = Student.objects.get(pk=17)

    print(f"ID: {student_id}\nFull Name: {get_student.first_name} {get_student.last_name}\nGPA: {get_student.gpa}")
...
def problem_seven(request):
    ...
    delete_student = Student.objects.filter(last_name="Bischoff").delete()
